...
# objdiff "answer key": a gitignored (asm/) target .s that defines each table
# symbol with its original bytes, so objdiff can name-pair it against the
# compiled gname_data.o base and confirm the data matches. This is only the
# grading target - the real build still compiles gname_data.c.
TARGET_ASM = REPO_ROOT / "asm" / "overlays" / "gname" / "gname_data.s"

# Symbols the C splits off the end of a table (must match gname_data.c so the
# target object pairs against the same base symbols).
TAIL_SYMBOLS = {"g_glyph_append_anim_frames": "gname_anim_frames_tail"}
...
def target_symbols(data: bytes):
    """Yield (symbol_name, bytes) matching gname_data.c's symbol boundaries,
    including any split-off tail symbol, for the objdiff target object."""
    for name, _, _, chunk in slices(data):
        spec = SPECS[name]
        count = len(chunk) // spec.elem
        main = chunk[: count * spec.elem]
        tail = chunk[count * spec.elem:]
        yield name, main
        if tail:
            yield TAIL_SYMBOLS[name], tail


def render_target_asm(data: bytes) -> str:
    """Render the objdiff target .s: one sized data symbol per table, bytes
    verbatim. gitignored (asm/), so no copyrighted bytes are committed."""
    lines = [
        "/* GENERATED by tools/gen_gname_data.py - objdiff target, do not commit. */",
        '.include "macro.inc"',
        "",
        '.section .data, "wa"',
        "",
    ]
    for name, b in target_symbols(data):
        lines.append(f"dlabel {name}")
        for i in range(0, len(b), 12):
            row = ", ".join(f"0x{x:02X}" for x in b[i:i + 12])
            lines.append(f"    .byte {row}")
        lines.append(f"enddlabel {name}")
        lines.append("")
    return "\n".join(lines) + "\n"
...
def main() -> None:
    ...
    TARGET_ASM.parent.mkdir(parents=True, exist_ok=True)
    TARGET_ASM.write_text(render_target_asm(data), newline="\n")
    print(f"Wrote objdiff target {TARGET_ASM.relative_to(REPO_ROOT)}")
