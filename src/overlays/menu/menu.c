...
void menu_update_slots(RenderContext* render_ctx)
{
    ...
    render_ctx->prim_cursor = menu_draw_frame(render_ctx->prim_cursor, &render_ctx->ot[MENU_FRAME_OT_INDEX], render_ctx->frame_parity, var_a3);

    if (g_menu_pending_overlay != 0)
    {
        render_ctx->prim_cursor = (void*)func_800A88A0(render_ctx->prim_cursor, &render_ctx->ot[MENU_FRAME_OT_INDEX], g_menu_pending_overlay, 1, 0xA0, 0xCA, 2);
    }
}
...
/**
 * @brief One byte of the party-sort order table for the current reorder screen.
 ...
 *       register. Measured: bare-ternary spellings cost 20 exact rows.
 */
#define MENU_SORT_SLOT(i)                                                                                                                                      \
    (sort_base = (u8*)g_pad_ctx, sort_off = ((g_menu_scene_type == 0x1F) ? 0 : 0x250), ((MenuPartyOrder*)(sort_base + sort_off))->order[i])
 ...
 * @see decomp.me (93.99%) https://decomp.me/scratch/DRmEd
 * @note The linked scratch is stale - it still holds the old 68.48% source, which
 *       was not functionally equivalent. This version is rebuilt from the target.
 *       Measured in isolation the same source scores 94.01%; the 6-row gap is
 *       objdiff naming the three jump tables differently in the two contexts,
 *       not a codegen difference.
 * @note Not yet byte-exact. The residual is register allocation: the target keeps
 *       one more callee-saved register (frame -0x80 vs -0x78), hoisting
 *       %hi(g_menu_cursor_enable) into the nav-loop preheader, and it uses eight
 *       distinct temps for the party-sort offset where this source has one.
 * @note Locals still carrying m2c-derived names (var_s0, var_s4, temp_s5, sp18,
 *       new_var7) are TODO renames; their declaration order must not change.
 */
void menu_handle_input(s32 process_actions)
{
    ...
    if (g_menu_scene_type < 0x11U && (g_pad_input & 0xF))
    {
        ...
        incdec_common:
            ...
            if (g_menu_hit_item_idx != (-1))
            {
                MenuContentItem* temp_s5 = g_menu_content_table[nodes[g_menu_scene_type].idx_nav.s.self_idx];
                g_content_view_x = temp_s5[g_menu_hit_item_idx].packed_x & 0x1FF;
                g_content_view_y = temp_s5[g_menu_hit_item_idx].y - 8;
                g_menu_suppress_cursor = 5;
                g_menu_cursor_enable = 1;
    ...
    if (process_actions != 0)
    {
        ...
        if ((g_menu_scene_type == 0x1F || g_menu_scene_type == 0x2B) && g_menu_hit_item_idx >= 0x11 && g_menu_hit_item_idx < 0x19)
        {
            ...
            if (g_pad_input & 0x220)
            {
                ...
                {
                    menu_play_se(0x7D, 0x80);
                    g_party_sort_marker.x = (s8)((s5[g_menu_hit_item_idx].packed_x & 0x1FF) - 2);
                ...
                else if (g_party_sort_marker.selected_idx != g_menu_hit_item_idx)
                {
                    ...
                    tmp = MENU_SORT_SLOT(idx_a);
                    MENU_SORT_SLOT(idx_a) = MENU_SORT_SLOT(idx_b);
                    MENU_SORT_SLOT(idx_b) = tmp;
                    g_party_sort_marker.selected_idx = 0xFF;
                }
                else
                {
        ...
        else if (g_pad_input & 0x220)
        {
            ...
            if (top_nibble == 0x5000)
            {
                ...
                case 6:
                    if (g_menu_char_slot == 0)
                    {
                        ...
                        flag = ((u8*)g_pad_ctx)[0x609 + content_type];
                    ...
                    if ((g_menu_scene_type >= 0x12) || (g_menu_scene_type == 1) || (g_menu_scene_type == 2))
                    {
                        if ((g_menu_item_ptr != ((void*)0)) && (*((u8*)g_menu_item_ptr) != 0))
                        {
                            ...
                            if (menu_item_is_nondefault(g_menu_item_ptr) != 0)
                            {
                                ...
                                func_800A8E28(sp18, (s8*)(base + (*((u16*)(base + (((*((u16*)((char*)g_menu_item_ptr + 0x16))) & 0x3F) * 2) + 0x48)))));
                                ...
                                case 0:
                                    base = (u8*)g_menu_state_ptr + (*((s32*)((char*)g_menu_state_ptr + 0x88)));
                                    menu_concat_encoded_text(&D_801226B8, sp18, (s8*)(base + (*((u16*)(base + ((item14 >> 9) & 0x7E))))));
                                    break;

                                case 1:
                                    base = (u8*)g_menu_state_ptr + (*((s32*)((char*)g_menu_state_ptr + 0x88)));
                                    menu_concat_encoded_text(&D_801226B8, sp18, (s8*)(base + (*((u16*)(base + ((item14 >> 9) & 0x7E) + 0x20)))));
                                    break;
                                ...
                                default:
                                    ...
                                    menu_concat_encoded_text(
                                        &D_801226B8, sp18,
                                        (s8*)(base + (*((u16*)(base + (((((u32)(*((u32*)((char*)g_menu_item_ptr + 0x14)))) >> 9) & 0x7E)) + 0x40)))));
                                    break;
                                }
                                g_menu_load_request = 1;
            ...
            else if (top_nibble == 0xF000)
            {
                ...
                if (val != 0)
                {
                    ...
                    {
                    case 1:
                        if (((u8*)g_pad_ctx)[(g_menu_char_slot * 0x250) + 0x5F0] != 0)
                        {
                            u8* rec;
                            s32 idx;
                            s32 count;

                            if ((((u8*)g_pad_ctx)[(g_menu_char_slot * 0x250) + 0x608] & 0x7F) == 4)
                            {
                                D_801229F4 = ((u8*)g_pad_ctx)[(g_menu_char_slot * 0x250) + 0x609] + 0x80;
                            }
                            else
                            {
                                D_801229F4 = ((u8*)g_pad_ctx)[(g_menu_char_slot * 0x250) + 0x609];
                            }
                    ...
                    case 10:
                        if (var_s4[0xAE] == 0xFF)
                        {
                            ...
                            g_menu_message_line2 = (u8*)g_menu_state_ptr + (*((s32*)((char*)g_menu_state_ptr + 8))) +
                                                   (*((u16*)((u8*)g_menu_state_ptr + (*((s32*)((char*)g_menu_state_ptr + 8))) + 0xCC)));
                            for (idx_a = 3; idx_a >= 0; idx_a--)
                            {
                                slots->active = 0;
                                slots--;
                            }
 ...
 *        (frame now matches; remaining: 4 cross-jump-cluster structural runs
 *        for func_800A88A0 call sites, one clock-select branch-polarity run,
 *        and the long-standing case-2 b1<2 mystery pair).
 * @see decomp.me (87.09%) https://decomp.me/scratch/D6Nba
 */
        ...
        else
        {
            ...
            loop_top:
            {
                ...
                case 2:
                {
                    ...
                    else if ((u32)(*(u8*)(var_s4 + 1) - 0x1A) < 8)
                    {
                        ...
                        if (((a2_5 >> (*(u8*)(var_s4 + 1) - 0x1A)) & 1) != 0)
                        {
                            t0 = D_80168696[*(u8*)(var_s4 + 1)];
                        }
                        else
                        {
                            t0 = 0x21;
                    ...
                    else if ((u32)(*(u8*)(var_s4 + 1) - 0x2A) < 8)
                    {
                        ...
                        while (idx < 8)
                        {
                            if (*(u8*)(ptr + idx + 0x638) == *(u8*)(var_s4 + 1) - 0x2A)
                            {
                                break;
                            }
                            idx++;
                    ...
                    if (t0 != 0xFF)
                    {
                        var_s1 = menu_emit_draw_mode_primitive(menu_emit_icon_sprite(var_s1, arg1, t0, *var_s3 & 0x1FF, *var_s4 - 8, 0, 0, 0, 0), arg1);
                    }
                }
                break;
                ...
                case 3:
                {
                    ...
                    case 0x2:
                        if (g_menu_item_ptr != 0)
                        {
                            ...
                            {
                                base2 = (void*)((u8*)g_menu_state_ptr + *(s32*)((u8*)g_menu_state_ptr + 0x68));
                                a2_6 = (void*)((u8*)base2 + *(u16*)((u8*)base2 + ((*(u32*)(g_menu_item_ptr + 0x14) >> 9) & 0x7E) + 0x2E));
                            }
                            menu_concat_encoded_text(&sp28, &sp68, a2_6);
                    ...
                    case 0x1E:
                    {
                        ...
                        if (tmp >= 0x1B)
                        {
                            ...
                            if (D_80168C01[tmp] != 0)
                            {
                                ...
                                if (delta2 >= 0)
                                {
                                    v0_6 = g_menu_label_key_a.page;
                                    a1_2 = g_menu_label_key_a.entry;
                                    v1_4 = (u8*)((u8*)&g_menu_label_key_a - 0x16);
                                }
                                else
                                {
                                    v0_6 = g_menu_label_key_b.page;
                                    a1_2 = g_menu_label_key_b.entry;
                                    v1_4 = (u8*)((u8*)&g_menu_label_key_b - 0x20);
                                }
                                tmp2 = (void*)(a1_2 + ((v0_6 << 8) + (s32)v1_4));
                    ...
                    case 0x4A:
                    {
                        ...
                        if (has)
                        {
                            if (total < 0)
                                total = -total;
                            var_s1 = func_800A8A78(arg1, var_s1, (u16)total, 1, pos_p, ((*var_s3 >> 9) & 7));
                        }
                    }
                    break;
                    default:
                ...
                case 4:
                {
                    ...
                    case 0x1:
                    {
                        ...
                        *(s8*)(prim + 3) = 4;
                        *(s8*)(prim + 7) = 0x64;
                        *(s16*)(prim + 8) = (s16)(*var_s3 & 0x1FF);
                        *(s16*)(prim + 10) = (s16)(*var_s4 - 8);
                        a0_5 = 0xD0;
                        if (g_menu_char_slot == 2)
                        {
                            a0_5 = 0xA0;
                        ...
                        *(s8*)(prim + 13) = v1_10;
                        *(s32*)(prim + 0x10) = 0x300030;
                        *(s16*)(prim + 14) = (s16)((((u16)g_menu_char_slot + 0x1D8) << 6) | 0x11);
                        var_arg1 = arg1;
                        *(s32*)var_s1 = (*(s32*)var_s1 & 0xFF000000) | (*var_arg1 & 0xFFFFFF);
                        temp_s1 = (void*)(prim + 0x14);
                        *var_arg1 = (*var_arg1 & 0xFF000000) | ((s32)temp_s1 & 0xFFFFFF);
                        *(s32*)((u8*)temp_s1 + 4) = 0;
                        *(s8*)((u8*)temp_s1 + 3) = 4;
                        *(s8*)((u8*)temp_s1 + 7) = 0x66;
                        *(s16*)((u8*)temp_s1 + 8) = (s16)((*var_s3 & 0x1FF) + 2);
                        *(s16*)((u8*)temp_s1 + 10) = (s16)(*var_s4 - 6);
                        a0_6 = 0xD0;
                        if (g_menu_char_slot == 2)
                        {
                            a0_6 = 0xA0;
                        ...
                        *(s32*)(prim + 0x14) = (*(s32*)(prim + 0x14) & 0xFF000000) | (*var_arg1 & 0xFFFFFF);
                        temp_s1_2 = (void*)((u8*)temp_s1 + 0x14);
                        *var_arg1 = (*var_arg1 & 0xFF000000) | ((s32)temp_s1 & 0xFFFFFF);
                        *(s8*)((u8*)temp_s1_2 + 3) = 1;
                        *(s32*)((u8*)temp_s1_2 + 4) = 0xE100001F;
                        link_p = (s32*)((u8*)temp_s1 + 0x14);
                        *link_p = (*link_p & 0xFF000000) | (*var_arg1 & 0xFFFFFF);
                        var_s1 = (void*)((u8*)temp_s1_2 + 8);
                        *var_arg1 = (*var_arg1 & 0xFF000000) | ((s32)temp_s1_2 & 0xFFFFFF);
                    }
                    break;
                    case 0x2:
                    {
                        ...
                        var_s1 = func_800A88A0(var_s1, var_arg1, a2_2, 1, *var_s3 & 0x1FF, *var_s4 - 8, (*var_s3 >> 9) & 7);
                    }
                    break;
                    case 0x3:
                    {
                        ...
                        var_s1 = menu_draw_clamped_number(var_arg1, var_s1, v, 1, pos_p, ((*var_s3 >> 9) & 7));
                    }
                    break;
                    case 0x4:
                        var_s1 = func_800A8A78(var_arg1, var_s1, *(s32*)((u8*)&D_80105AE0 + g_menu_char_slot * 0x23C + 4), 1, pos_p, ((*var_s3 >> 9) & 7));
                        break;
                    case 0x5:
                    {
                        void* base = *(void**)(pad_base + 0x271C);
                        u16 v = *(u16*)((u8*)base + g_menu_char_slot * 0x250 + 0x614);
                        var_s1 = func_800A8A78(var_arg1, var_s1, v, 1, pos_p, ((*var_s3 >> 9) & 7));
                    }
                    break;
                    case 0x6:
                    {
                        ...
                        var_s1 = func_800A8A78(var_arg1, var_s1, (u16)(v >> 8), 1, pos_p, ((*var_s3 >> 9) & 7));
                    }
                    break;
                    case 0x7:
                    case 0x8:
                    ...
                    case 0xE:
                    {
                        ...
                        var_s1 = menu_draw_clamped_number(var_arg1, var_s1, v >> 9, 1, pos_p, ((*var_s3 >> 9) & 7));
                    }
                    break;
                    case 0xF:
                    {
                        ...
                        if (*(u8*)a2_2 != 0)
                        {
                            ...
                            if (g_item_slot_flags.slot0 != 0)
                                a3 = 2;
                            var_s1 = func_800A88A0(var_s1, var_arg1, a2_2, a3, *var_s3 & 0x1FF, *var_s4 - 8, (*var_s3 >> 9) & 7);
                        }
                    }
                    break;
                    case 0x10:
                    ...
                    case 0x12:
                        if (*(u8*)D_80168C30 != 0)
                        {
                            ...
                            var_a2 = (void*)((u8*)g_menu_state_ptr + *(s32*)((u8*)g_menu_state_ptr + 0x64));
                            {
                                ...
                                var_s1 = func_800A88A0(var_a0, var_arg1, a2_2, 1, *var_s3 & 0x1FF, *var_s4 - 8, (*var_s3 >> 9) & 7);
                            }
                        }
                        break;
                    case 0x13:
                    {
                        ...
                        var_s1 = func_800A8A78(var_arg1, var_s1, a2_15, 1, pos_p, ((*var_s3 >> 9) & 7));
                    }
                    break;
                    case 0x14:
                    case 0x15:
                    ...
                    case 0x19:
                    {
                        ...
                        var_s1 = func_800A8A78(var_arg1, var_s1, (u16)(v - shift), 1, pos_p, ((*var_s3 >> 9) & 7));
                    }
                    break;
                    case 0x1B:
                    case 0x1C:
                    ...
                    case 0x20:
                    {
                        ...
                        var_s1 = menu_draw_clamped_number(var_arg1, var_s1, total, 1, pos_p, ((*var_s3 >> 9) & 7));
                    }
                    break;
                    case 0x21:
                    {
                    ...
                    case 0x3F:
                    {
                        if (g_item_slot_flags.slot0 != 0)
                        {
                            ...
                            ((u8*)&spB0)[func_800A8DDC(tmp2)] = 0;
                            var_s1 = func_800A88A0(var_s1, var_arg1, &spB0, 1, pos.x, pos.y, 0);
                        }
                    }
                    break;
                    case 0x40:
                    ...
                    case 0x44:
                    {
                        if (g_item_slot_flags.slot0 != 0)
                        {
                            ...
                            if (diff < 0)
                                diff = -diff;
                            var_s1 = func_800A8A78(var_arg1, var_s1, (u16)diff, 1, pos_p, ((*var_s3 >> 9) & 7));
                        }
                    }
                    break;
                    case 0x45:
                    ...
                    case 0x48:
                    {
                        ...
                        if (has)
                        {
                            if (total < 0)
                                total = -total;
                            var_s1 = menu_draw_clamped_number(var_arg1, var_s1, (u32)total, 1, pos_p, ((*var_s3 >> 9) & 7));
                        }
                    }
                    break;
                    case 0x49:
                    ...
                    case 0x4A:
                    {
                        ...
                        if (v1 >= 0)
                        {
                            ...
                            var_s1 =
                                func_800A88A0(var_s1, var_arg1, (void*)((u8*)a2 + *(u16*)((u8*)a2 + (v * 2))), 1, *var_s3 & 0x1FF, *var_s4 - 8, (*var_s3 >> 9) & 7);
                        }
                    }
                    break;
                    case 0x4B:
                    ...
                    case 0x55:
                    {
                        ...
                        {
                        case 0x4E:
                            a2_26 = *(u32*)((u8*)*data_pp + g_menu_char_slot * 0x250 + 0x658) & 0xF;
                            break;
                        case 0x4F:
                            a2_26 = (*(u8*)((u8*)*data_pp + g_menu_char_slot * 0x250 + 0x658) >> 4);
                            break;
                        case 0x50:
                            a2_26 = (*(u32*)((u8*)*data_pp + g_menu_char_slot * 0x250 + 0x658) >> 8) & 0xF;
                            break;
                        case 0x51:
                            a2_26 = (*(u32*)((u8*)*data_pp + g_menu_char_slot * 0x250 + 0x658) >> 12) & 0xF;
                            break;
                        case 0x52:
                            a2_26 = *(u16*)((u8*)*data_pp + g_menu_char_slot * 0x250 + 0x65A) & 0xF;
                            break;
                        case 0x53:
                            a2_26 = (*(u32*)((u8*)*data_pp + g_menu_char_slot * 0x250 + 0x658) >> 20) & 0xF;
                            break;
                        case 0x54:
                            a2_26 = *(u8*)((u8*)*data_pp + g_menu_char_slot * 0x250 + 0x65B) & 0xF;
                            break;
                        case 0x55:
                            a2_26 = *(u32*)((u8*)*data_pp + g_menu_char_slot * 0x250 + 0x658) >> 28;
                            break;
                        }
                        var_s1 = menu_draw_clamped_number(var_arg1, var_s1, a2_26, 1, pos_p, ((*var_s3 >> 9) & 7));
                    }
                    break;
                    default:
                        break;
                ...
                case 6:
                {
                    ...
                    case 1:
                        ...
                        {
                            var_s1 = func_800A8A78(var_arg1, var_s1, 0x989680U, 1, pos_p, ((*var_s3 >> 9) & 7));
                        }
                        else
                        {
                            var_s1 = func_800A8A78(var_arg1, var_s1, *(u32*)(*(void**)(pad_base + 0x271C) + 0x2C), 1, pos_p, ((*var_s3 >> 9) & 7));
                        }
                        break;
                    case 2:
                    {
                        ...
                        pos.x += 0x14;
                        s2 = quotient >> 5;
                        var_s1_2 = (void*)func_800A8A78(var_arg1, var_s1, (u16)s2, 1, pos_p, 1);
                        if ((g_frame_counter / 15) & 1)
                        {
                            var_s1_2 = func_800A88A0(var_s1_2, var_arg1, ":", 1, pos.x, pos.y, 0);
                        }
                        pos.x += 7;
                        split_tmp = s2 * 0x3C;
                        remainder = (diff / 3600) - split_tmp;
                        if (remainder < 0xA)
                            var_s1_2 = (void*)func_800A8A78(var_arg1, var_s1_2, 0U, 1, pos_p, 0);
                        pos.x += 0x10;
                        var_s1 = (void*)func_800A8A78(var_arg1, var_s1_2, remainder, 1, pos_p, 1);
                    }
                    break;
                    }
                }
...

            {
                ...
                if (node->label_id == 0x13)
                {
                    ...
                    if (*a2_27 != 0)
                    {
                        var_s1 = func_800A88A0(var_s1, var_arg1, a2_27, 1, 0xAC, 0xC, 2);
                    }
                }
            ...
            case 27:
            {
                ...
                {
                    ltemp = func_800AD208(var_arg1, var_s1, g_script_repeat_last + 1, 3, &pos, 0);
                }
                else
                {
                    ltemp = func_800AD208(var_arg1, var_s1, 0, 3, &pos, 0);
                }
                ltemp = func_800AD524(ltemp, var_arg1, 0xB, &pos, 0);
                pos.x += 8;
                ltemp = func_800AD208(var_arg1, ltemp, g_menu_page_count, 3, &pos, 0);
                ltemp = func_800AD524(ltemp, var_arg1, 0xB, &pos, 0);
                pos.x += 8;
                var_s1 = func_800AD208(var_arg1, ltemp, menu_count_inventory_items(), 3, &pos, 0);
            }
            break;
            default:
                break;
        ...
        do
        {
            if ((node2->u2.unk2 >> 1) & 1)
            {
                ...
                for (; j < 4; j++)
                {
                    ...
                    buf = menu_draw_node_recursive(child, buf, arg2);
                    sentinel = 0;
                }
            }
        } while (0);
...
s32 func_8014C200(s32* ot, ScrollListState* state, s32 prim_buf, Vec2s* view_origin, int active)
{
    ...
    prim_buf = scroll_list_draw(prim_buf, ot, list, g_menu_item_nav_entries, view_origin, active);

    prim_buf = func_800A88A0(prim_buf, ot, MENU_TAIL(MENU_STATE_BASE(8), 0x7A), 1, 0x30 - view_origin->x, -view_origin->y, 2);
    prim_buf = func_800A88A0(prim_buf, ot, MENU_TAIL(MENU_STATE_BASE(8), 0x7C), 1, 0x30 - view_origin->x, 0x10 - view_origin->y, 2);
    prim_buf = func_800A88A0(prim_buf, ot, MENU_TAIL(MENU_STATE_BASE(8), 0x86), 1, 0x30 - view_origin->x, 0x20 - view_origin->y, 2);
    prim_buf = func_800A88A0(prim_buf, ot, MENU_TAIL(MENU_STATE_BASE(8), 0x80), 1, 0x30 - view_origin->x, 0x30 - view_origin->y, 2);
    return prim_buf;
}
