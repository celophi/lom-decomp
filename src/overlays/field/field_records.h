...
typedef struct FieldItemRecord
{
    ...
    u8 attributes[4];
    s32 handle;
    /** @brief Unique key, generated by func_800C37A8 when the item is created. */
    FieldItemKey key;
} FieldItemRecord;
...
/** @brief FieldLandRecord::position of a land that is not on the map. */
#define FIELD_LAND_POSITION_NONE 0xFF
...
typedef struct FieldLandRecord
{
    ...
    u8 position;
    u8 unk2;
    /** @brief Placement order: FieldGameState placed_land_count when the land was placed. */
    u8 count;
    u8 levels[8];
} FieldLandRecord;
...
/**
 * @brief View of a stored companion record shifted by @p bytes bytes.
 ...
 */
#define FIELD_REGION_AT(record, bytes) ((FieldRegionRecord*)((u8*)(record) + (bytes)))
...
typedef struct FieldActorRecord
{
    u8 id;
    u8 selector;
    /** @brief Pickup handed over by field_grant_actor_pickup: bit 15 selects a counter, else a reward key. */
    u16 pickup;
