...
void movie_play(s32 movie_index)
{
    ...
    while (TRUE)
    {
        ...
        state->frame_ready = 0;
        func_800157B0(4);
        new_var = (u16)(movie_index & 0xFFFF);
        VSync(0);
        p_disp_env = &env[0];
...
void movie_init(s32 resource_index, s32 flags, s32 total_frames, s32 init_buffer_idx)
{
    ...
    else
    {
        ...
        new_var = (unsigned short)((MovieState*)0x801ED500)->rects[init_buffer_idx].y;
    ...
    if (g_mdecRetryPending != 0)
    {
        if ((MOVIE_STATE->mdec_busy == 0) && (combined->frame_ready == 0))
        {
                     ...
                     (MOVIE_STATE->gpu_mode & 0xFFFFu) == 0);
            {
                ...
                DecDCTout((((MovieState*)MOVIE_STATE)->mdec_output_buf[MOVIE_STATE->out_buf_idx]), word_count >> 1);
            }
            MOVIE_STATE->mdec_retry_pending = 0;
        }
    }
...
s32 movie_cd_sector_callback(void)
{
    ...
    else if (MOVIE_STATE->continuation_type == 0)
    {
        ...
        temp_s1 = (u32*)((u8*)MOVIE_STATE->video_table_base + ((VOL_MOVIE_STATE->video_write_idx + MOVIE_STATE->chunk_sector_idx) << 5));
    ...
    else
    {
        ...
        madr = (u8*)MOVIE_STATE->audio_data_base + ((VOL_MOVIE_STATE->audio_write_idx + MOVIE_STATE->chunk_sector_idx) << 0xB);
